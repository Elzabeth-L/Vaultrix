...
app.get('/invoices/order/:orderId', async (req, res) => {
    try {
        let invoice = await Invoice.findOne({ orderId: req.params.orderId });
        if (invoice) return ok(res, { invoice });

        if (typeof fetch !== 'function') return err(res, 'Invoice not found', 404);

        const orderRes = await fetch(`${ORDER_SERVICE_URL}/orders/${req.params.orderId}`);
        if (!orderRes.ok) return err(res, 'Invoice not found', 404);
