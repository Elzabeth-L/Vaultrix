...
// ── POST /invoices — Internal: create invoice after payment ─────────────────
...
// ── GET /invoices/:id/download — Download professional PDF invoice ───────────
app.get('/invoices/:id/download', async (req, res) => {
    try {
        ...
        doc.text(`Rs. ${Number(invoice.amount).toFixed(2)}`, 490, 304, { align: 'right' });

        // ── Total ──────────────────────────────────────────────────────────
        doc.rect(350, 340, 195, 55).fill('#1e1e2e');
        doc.fillColor('#94a3b8').fontSize(9).font('Helvetica').text('TOTAL AMOUNT PAID', 365, 352);
        doc.fillColor('#10b981').fontSize(20).font('Helvetica-Bold').text(`Rs. ${Number(invoice.amount).toFixed(2)}`, 365, 367);
